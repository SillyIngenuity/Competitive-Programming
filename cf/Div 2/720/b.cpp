...
void solve() {
  ...
  vi arr(n);
  rep (i,n) cin >> arr[i];
  int minindex = (int)mini(arr) + 1;
  int minelement = mine(arr);

  debug(minindex, minelement);
  cout << n - 1 << '\n';
  ...
  rep (i,1, n + 1) {
    ...
    // x will always be abs(currentposition - mini) + mine
    // For debugging purposes i shall print out the arr
    // arr[i] = abs(i-minindex) + minelement;
    // cout << minindex << " " << i << " "<< minelement << " " << abs(i-minindex) + minelement << '\n';

    cout << i << " " << minindex << " " << abs(i - minindex) + minelement << " " << minelement << '\n';
  }
  // print_v(arr);
