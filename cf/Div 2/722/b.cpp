...
int dy[4] = {0,1,0,-1};
void solve() {
  // Easy to prove that there can only exist one postive int
  // For the sake of contradiction we assume theres two positive ints a,b
  // Assume a is the max
  // abs(a-b) >= a is a contradiction
  ...
  // determine if you can have a positive integer
  // Compute the smallest gap in the negatives array
  ...
  int gap = 1e9;
  rep (i, sz(negatives) - 1) {
    int temp = abs(negatives[i] - negatives[i + 1]);
    gap = min(gap,temp);
  }
  int sum = sz(negatives);

  if (sz(positives) >= 1 && gap >= positives[0]) {
    sum +=1;
  }
  cout << sum << '\n';
  return;
